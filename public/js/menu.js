        ...
        for (let i = 0; i < orders[0].pesanan.length; i++) {
            if (orders[0].pesanan[i] != null) {
                if (orders[0].pesanan[i].id == idmenu) {
                    ...
                    $(btn_hilang).attr('class','btn btn-primary me-auto hilang');
                    btn_muncul.querySelector('.qty-pesan').value = orders[0].pesanan[i].qty;
                    temp_Order.push(composeMenu(orders[0].pesanan[i].id,orders[0].pesanan[i].nama,orders[0].pesanan[i].qty,orders[0].pesanan[i].harga,orders[0].pesanan[i].foto))
...
$('#next').click(function (){
    ...
    if(nama === "" || meja === "" || temp_Order.length == 0){
        Swal.fire({
            icon: "error",
        ...
        orders.push(composeDatapesanan(nama,meja,temp_Order,tgl_pesan));
