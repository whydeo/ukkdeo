...
if(orders[0].pesanan.length > 0){
  ...
  for (let i = 0; i < orders[0].pesanan.length ; i++) {
    if (orders[0].pesanan[i] != null) {
      TOTAL_PEMBAYARAN += orders[0].pesanan[i].qty * orders[0].pesanan[i].harga;
      list.append(makeorder(orders[0].pesanan[i].nama,orders[0].pesanan[i].foto,orders[0].pesanan[i].harga,orders[0].pesanan[i].qty,i))
    }
...
bayar.addEventListener('click',function () { 
  Swal.fire({
    title: 'Pembayaran',
    input: 'text',
    inputLabel: `Total Pembayaran: Rp ${TOTAL_PEMBAYARAN}` ,
